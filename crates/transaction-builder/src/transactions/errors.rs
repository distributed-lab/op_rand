...
impl fmt::Display for TransactionError {
    fn fmt(&self, f: &mut fmt::Formatter<>) -> fmt::Result {
        match *self {
            ...
            TransactionError::Secp256k1(ref err) => f.write_str(
                &format!("Bitcoin Secp256k1 error: {}", err),
            ),
            TransactionError::Psbt(ref err) => f.write_str(
                &format!("Bitcoin PSBT error: {}", err),
            ),
            TransactionError::NoDepositTxStored => f.write_str(
                "TransactionBuilder must store some deposit transaction."
            ),
            TransactionError::AmountsScriptsLengthMismatch => f.write_str(
                "Amounts and scripts must be of same length."
            ),
            TransactionError::InputsOutputsLengthMismatch => f.write_str(
                "Inputs and outputs must be of same length."
            ),
            TransactionError::TransactionTypeMismatch => f.write_str(
                "Transaction is a different type from expected."
            ),
            TransactionError::InputIndexOutOfBounds => f.write_str(
                "Provided input index is out of bounds for this transaction."
            ),
        }
    }
}
